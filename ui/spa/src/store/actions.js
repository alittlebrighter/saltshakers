...
export const getHouseholds = () =>
  fetch(config.apiBaseURL + "/households", { method: "GET", mode: "cors" })
    .then(response => response.json())
    .then(households => setHouseholds(households));

export const getHousehold = hhId =>
  fetch(config.apiBaseURL + "/households/" + hhId)
    .then(response => response.json())
    .then(hh => upsertHousehold(hh));

export const saveHousehold = hh =>
  fetch(config.apiBaseURL + "/households", {
    method: "POST",
    headers: {
...
export const deleteHousehold = id =>
  fetch(config.apiBaseURL + "/households/" + id, { method: "DELETE" }).then(
    () => removeHousehold(id)
  );

export const generateGroups = hhCount =>
  fetch(config.apiBaseURL + "/groups/generate?targetHouseholdCount=" + hhCount)
    .then(response => response.json())
